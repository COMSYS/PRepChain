...
#@profile
def main(iteration, eval_run, voter, votee, x,y,z,complete_selected_ratings, complete_types, limits, random_sub_vote):
    ...
    performance_dict['voter_pseudonym_recent_request_perf'] = round(t2[0]-t1[0],2)
    ...
    t2 = time.perf_counter(), time.process_time()
    performance_dict['votee_pseudonym_recent_request_perf'] = round(t2[0] - t1[0], 2)
    ...
    t2 = time.perf_counter(), time.process_time()
    performance_dict['verification_pseudonym_recent_request_perf'] = round(t2[0] - t1[0], 2)
    ...
    if votee_reputation_request["total_count"] == 0:
        ...
        t2 = time.perf_counter(), time.process_time()
        performance_dict['HE_votee.keyGen()_perf'] = round(t2[0] - t1[0], 2)
        ...
        if voter_reputation_request["total_count"] == 0:
            ...
            t2 = time.perf_counter(), time.process_time()
            performance_dict['sk_send_request_perf'] = round(t2[0] - t1[0], 2)
            ...
            rating_request = send_ratinginfo(args.voterSystemID, args.voteeSystemID, padded_rating, padded_count,
                                             voter_reputation_request["R_recent"],
                                             voter_pseudonym_recent["recentpseudonym"],
                                             db_ratingid["voteid"], voter_enc_json, verification_request['signature_rating'],
                                             verification_request['signature_count'], eval_run, iteration).json()
            t2 = time.perf_counter(), time.process_time()
            performance_dict['rating_request_perf'] = round(t2[0] - t1[0], 2)
            #performance_dict['rating_request_proc'] = round(t2[1] - t1[1], 2)
        ...
        sk_get = get_secret_key(args.voteeSystemID, eval_run, iteration).json()
...
def check_equivalence(votee, upd_rep):
    ...
    for i in range(min(len(enc_rating), len(rating[upd_rep]["sub"]["rating"]))):
        if np.round(enc_rating[i],1) != round(rating[upd_rep]["sub"]["rating"][i],1):
    ...
    for i in range(min(len(enc_count), len(rating[upd_rep]["sub"]["count"]))):
        if np.round(enc_count[i],1) != round(rating[upd_rep]["sub"]["count"][i],1):
    ...
    return True, enc_rating.tolist(), enc_count.tolist(), rating[upd_rep]['sub']['rating'], rating[upd_rep]['sub']['count']

if __name__ == "__main__":
    # Define the evaluation name
    evaluation_name = 'eval4'
    ...
    START_USER = 20001
    END_USER = 35000  # This can be adjusted if needed
    BATCH_SIZE = 100

    data_file_path = os.path.join(evaluation_folder, 'data.pkl')
    if os.path.exists(data_file_path):
        ...
        current_end = START_USER + BATCH_SIZE - 1
        users = {i: 0 for i in range(current_start, current_end + 1)}
        iteration = 0
    ...
    for scenario in range(4):
        for run in range(0,20):
            ...
            sub = config['Rating']['sub_num']
            obj = config['Rating']['obj_num']
            tru = config['Rating']['tru_num']

            if scenario == 0:
                # Scenario for both users with rep = 0
                user1 = next((k for k, v in users.items() if v == 0), None)
                user2 = next((k for k, v in users.items() if v == 0 and k > user1), None)
                if user2 is not None:
                    users[user2] += 1
            elif scenario == 1:
                # Scenario for first user with rep = 0 and second has rep > 0
                user1 = next((k for k, v in users.items() if v == 0), None)
                user2 = next((k for k, v in users.items() if v > 0), None)
                if user2 is not None:
                    users[user2] += 1
            elif scenario == 2:
                # Scenario for first user with rep > 0 and second has rep = 0
            ...
            complete_selected_ratings = config["Rating"]["sub_fields"][:sub] + config["Rating"]["obj_fields"][:obj] + config["Rating"]["tru_fields"][:tru]
            #complete_types = ["diff"] * (obj + tru)
            complete_types = ["diff",[21.01, 23.87],[0.08, 0.14],[-37.5, 150]]
            limits = [100] * (obj + tru)
            random_sub_vote = [1] * sub
            print(iteration, evaluation_name, user1, user2, sub, obj, tru, complete_selected_ratings, complete_types, limits, random_sub_vote)

            votee, rating, performance_dict = main(iteration, evaluation_name, user1, user2, sub, obj, tru, complete_selected_ratings, complete_types, limits, random_sub_vote)
            ...
            # Define the path to the "performancedictpkl" folder within "evaluation_folder"
            """performancedictpkl_folder = os.path.join(evaluation_folder, 'performancedictpkl')
            os.makedirs(performancedictpkl_folder, exist_ok=True)

            performance_pickle_path = os.path.join(performancedictpkl_folder, f'performance_dict_{iteration}.pkl')
            with open(performance_pickle_path, 'wb') as perf_file:
                pickle.dump(performance_dict, perf_file)"""
            ...
            known_columns = ['eval',
                            ...
                            'rating_types',
                            'rating_limits',
                            'voter_pseudonym_recent_request_perf',
                            'votee_pseudonym_recent_request_perf',
                            'verification_pseudonym_recent_request_perf',
                            'voter_reputation_request_perf',
                            'votee_reputation_request_perf',
                            'HE_votee.keyGen()_perf',
                            ...
                            'size_s_enc_rating',
                            'size_s_enc_count',
                            'verification_request_perf',
                            'rating_request_perf',
                            'sk_send_request_perf',
